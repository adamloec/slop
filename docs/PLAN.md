...
| **E5** | Cascaded shadow maps | E6 | **Prerequisite landed**: the directional light is data — `Environment`, a per-frame buffer carrying the sun and the ambient term — because cascades are built along a direction the CPU has to know. Both references bit-identical across the move. Still to do: the cascade split, the four light-space matrices, the depth-only array render, and the comparison sample in the forward pass |
