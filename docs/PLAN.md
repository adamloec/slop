...
| **E4** | Clustered forward+ — light list, cluster build, forward pass | E5 | The first compute-feeding-graphics dependency. **Two prerequisites landed**: the graph expresses compute passes and tracks buffers, and the depth prepass now runs — depth-only passes are expressible and all six goldens are unchanged with one in the frame. It also found a real barrier bug: two passes in the *same* state need one anyway, and synchronization validation did not report it. Still to do — the cluster grid, the light list, and the forward pass reading it |
