...
| ~~The tangent is transformed by the normal matrix, not the model matrix~~ | `shaders/passes/scene/model.slang` | — | **Resolved at E4**, and by the route this row predicted. Point lights need world position, world position needs the model matrix, and the push block had no room — so per-instance data went into a storage buffer, and the tangent now uses the model matrix that arrived with it. The block dropped from 120 bytes to 88 in the process, since the model-view-projection and the normal matrix both moved into the instance row. Neither reference image moved: an unlit render of both models is bit-identical to the pre-lighting references, which is what says the transform rewrite was exact. | ✅ |
...
| The environment BRDF is an analytic fit, not a lookup table | `shaders/lib/lighting/brdf.slang` | The 128² RG16F table the split-sum approximation was published with | **Replaced, and behind an unchanged seam.** `environmentBrdf` depends on roughness and *N·V* alone — nothing about the environment — so a cooked table would be one artifact shared by every scene, which is a second asset kind and a second upload path for what fits in ten instructions. Lazarov's fit is accurate to a fraction of a percent over the whole domain and is what several shipped titles use. If a measurement ever shows it costing something visible, the table replaces one function and no caller changes. | M3+ |
| Nothing checks that shading conserves energy | `shaders/passes/scene/model.slang` | A white furnace test | **Extended, and §9.7 E6d promised this and did not deliver it.** With a uniform environment of unit radiance and no direct light, a surface should reflect its own albedo back — that is what catches a missing `1 - F`, a double-counted term, or a specular lobe that gains energy, none of which a reference image distinguishes from a lighting choice. What blocks it is that the harness renders **cooked models**, so the test does not know any surface's albedo, roughness or metallic; it needs a synthetic material, which is a fixture that does not exist. The split-sum approximation also loses energy at high roughness by construction, so the assertion is a band rather than an equality, and picking the band needs the fixture first. | M3 |
...
| A cluster silently drops lights past its stride | `shaders/passes/scene/cluster_build.slang` | Either a compacted list, or a reported overflow | **Extended.** A cell reached by more than `max_per_cluster` lights keeps the first 64 in buffer order and drops the rest, and *which* it drops therefore depends on light order. Nothing reports it. At four lights this cannot fire; at the light counts §9.4 is aimed at, it can. The compaction row above is the fix, and a counter is the cheap interim. | M3, E7 |
| The cluster build reads every light per cell | `shaders/passes/scene/cluster_build.slang` | A coarse cull per tile column first | **Replaced.** 3456 clusters × the light count, every frame. Fine at the scene sizes here and the standard next step when it is not. Behind an unchanged declaration — the pass's inputs and outputs do not change. | M3, E7 |
| The grid is a fixed 16 × 9 × 24 regardless of aspect | `slop-render/src/lighting/cluster.rs`, the examples | Tiles derived from the target's size | **Extended.** Tiles stay square only at 16:9. A window at another ratio gets stretched cells, which costs efficiency rather than correctness — a cell covering more screen lists more lights. The fields are already per-instance, so this is a caller change. | M3 |
...
| The cook cache never removes an artifact whose source is gone | `slop-asset/src/cache.rs` | A sweep that deletes artifacts no source claims | **Extended, and found the hard way.** Cooking is keyed on content, so a *changed* source recooks correctly — but a source that is renamed, moved or deleted leaves its artifact and stamp in `.slop/cache` forever, and `Vfs::read` of the old logical path keeps succeeding. Found by reorganising `shaders/`: the cache afterwards held both `passes/model.spv` and `passes/scene/model.spv`, so every test would have passed reading artifacts from a layout that no longer existed. The reorg was verified by deleting the cache and recooking, which is the workaround and is not one anybody will remember. A cook run knows every logical path it wrote; what is missing is the sweep that removes the rest. | M3 |
...
#### What E6d found: the shader path the tests do not run

The reorganisation two commits earlier moved `model.slang` to
`passes/scene/model.slang` and rewrote every logical path — and the windowed
viewer builds two of its paths with `format!("shaders/passes/{name}.spv")`,
which no rewrite of a literal could catch. **The entire suite stayed green**,
because the golden harness names its shaders directly and the windowed path has
no automated coverage at all.

Found by running `SLOP_FRAMES=5` by hand, which is what §6.1's `FrameRenderer`
row already says is the only check that path gets. It is now a second reason to
close that row: the resize path being untested is one gap, and this is another
of the same shape — the demo refusing to start while the suite reports green is
precisely the failure the golden suite's named-skip discipline exists to
prevent, arriving through a door the skips do not cover.
