...
| A graph pass must have a colour attachment | `slop-render/src/graph.rs` | Compute passes, declared the same way | **Extended.** `PassDesc` describes a rendering pass, and §9.4's cluster build is a dispatch — it has no attachment, and needs storage-image and storage-buffer accesses the declaration cannot express. E4 is what adds them, and it is the first consumer that would say what the shape should be. Declaring one now panics by name rather than silently skipping. | M3, E4 |
