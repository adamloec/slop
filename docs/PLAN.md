...
| The directional light is still a constant in the shader | `shaders/passes/model.slang` | Data, alongside the point lights | **Replaced.** It has no position and infinite extent, so it belongs to every cluster by construction and clustering it is meaningless — making it data now would be moving it for no reader. E5 is the reader: cascaded shadows need its direction. | M3, E5 |
| The forward pass loops over every light | `shaders/passes/model.slang` | The cluster's own light list | **Replaced**, and this is the rest of E4 rather than a deferral. The buffer, the falloff and the radius are in place precisely so the loop body does not change when clustering lands — only where the indices come from. | M3, E4 |
...
| **E4** | Clustered forward+ — light list, cluster build, forward pass | E5 | The first compute-feeding-graphics dependency. **Three prerequisites landed**: the graph expresses compute passes and tracks buffers; the depth prepass runs; and lights are data — a storage buffer of point lights with a windowed inverse-square falloff, which is what makes a *radius* the thing cluster assignment can test. Forcing the per-instance buffer on the way, since a point light needs world position and the push block had no room for a model matrix. The prepass found a real barrier bug: two passes in the *same* state need one anyway, and synchronization validation did not report it. Still to do — the cluster grid, the cluster build pass, and the forward pass reading its own cell instead of every light |
